...
#      do
      cd $dir
      if [ ! -s $outdir/flx.$year$mon.avrg.grib ] ; then
	  ...
	  while [ $day -le $dayf ] ; do
	      ...
	      if [ ! -s flx.$year$mon$dd.avrg.grib ] ; then
		  echo file flx.$year$mon$dd.avrg.grib does not exist
		  exit 8
	      fi
	      echo flx.$year$mon$dd.avrg.grib >>list.$$
              day=`expr $day + 1`
	  done
#
	  ${GRMEAN} -s list.$$ -o $outdir/flx.$year$mon.avrg.grib || exit 8
	  ${FORCE_GRIB_DATE_MON} $outdir/flx.$year$mon.avrg.grib $year${mon}0100 || exit 8
      else
	  echo flx.$year$mon.avrg.grib exists
      fi
    done
    year=`expr $year + 1`
done
